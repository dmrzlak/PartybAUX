...
public class CustomProgressBar extends RelativeLayout{
    ...
    public void start(){
        ...
        progress.animate().alpha(1f).setDuration(150).start();
        progressAnim.start();
    }
    ...
    private void commitFinish(){
        ...
        if(finishAnim != null)
            finishAnim.start();
        final ViewPropertyAnimator alphaAnim = progress.animate().alpha(0f).setStartDelay(1200).setDuration(250);
        alphaAnim.setListener(new AnimatorListenerAdapter() {
            @Override
